...
deny contains msg if {
	...
	not bucket_has_default_tags(bucket)
	msg := sprintf("aws_s3_bucket %s does not have all default tags enabled", [bucket.address])
}

bucket_has_default_tags(bucket) if {
	tags := bucket.values.tags_all
