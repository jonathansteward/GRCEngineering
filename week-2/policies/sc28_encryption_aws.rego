...
deny contains msg if {
	...
	msg := sprintf("aws_s3_bucket %s has no server-side encryption configuration", [bucket.name])
}
...
bucket_has_encryption(bucket) if {
	...
	reference := enc.expressions.bucket.references[_]
	reference == sprintf("aws_s3_bucket.%s.id", [bucket.name])
}
